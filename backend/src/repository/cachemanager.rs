...
pub struct CacheManager {
    ...
    settings: Cache<String, bool>,
    db: PostgresRepository
}

impl CacheManager {
    pub fn new(db: PostgresRepository) -> CacheManager {
        return CacheManager {
            trade: Cache::builder()
            ...
            subscriptions: Cache::builder()
                .time_to_live(Duration::from_secs(60 * 60))
                .build(),
            settings: Cache::builder().build(),
            db: db
        };
    }

    pub fn is_cache_filled(&self) -> bool {
        match self.settings.contains_key("filled") {
            true => self.settings.get(r#"filled"#).unwrap(),
            false => {
                self.settings.insert("filled".into(), false);
                let _ = self.fill_cache();
                false
            }
        }
    }
    ...
    pub async fn fill_cache(&self) {
        match self.db.get_trades("60", "all").await {
            Some(trades) => {
                info!("Filling Cache");
                trades.iter().for_each(|trade| self.add_trade(trade.to_owned()));
                self.settings.insert("filled".into(), true);
                info!("Getting updated cache setting {:?}", self.settings.get("filled"));
            }, 
            None => {
                warn!("Failed to fill cache");
            }
        }
    }

    pub fn get_trades(&self, symbol: &str, interval: &str) -> Option<Vec<Trade>> {
        info!("Calling Cache get trades");
