...
pub async fn get_symbols(
    ...
    let result = symbol_service::get_symbols(&app_state.pool).await;
...
pub async fn get_add_symbol(
    ...
    let result = symbol_service::add_subscription_symbol(&app_state.pool, &app_state.sender, symbol).await;
   prepare_response(result)
}
